...
def consumable_sold_most(consumable):
	with engine.connect() as con:
        consumable_sold_most = sql.text("SELECT bar, count(consumable) AS num_sold FROM Bills WHERE consumable=:c GROUP BY bar ORDER BY count(consumable) DESC LIMIT 5;")
